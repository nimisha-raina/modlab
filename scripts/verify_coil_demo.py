...
assert all(abs(p.co.y-demo.CENTER[1])<1e-6 and abs(p.co.z-demo.CENTER[2])<1e-6 for p in keys.key_blocks[0].data)
for name,points in (("Ten turns",demo.wire_points(1)),("Twenty turns",demo.dense_wire_points(1))):
...
assert len(guides)==10 and len(extra)==14
...
for seconds,amps,count,left_pole in ((0,0,0,None),(14,0,0,None),(26,.5,10,"N"),
                                   (36,0,0,None),(42,-.5,10,"S"),(53,0,0,None),
                                   (61,-.5,16,"S"),(75,0,0,None),(93,-.5,24,"S"),
                                   (102,0,0,None),(108,-.5,24,"S"),(113,0,0,None)):
    scene.frame_set(round(seconds*demo.FPS)+1)
    bpy.context.view_layer.update()
    check_ammeter(scene,amps,framed=seconds in (26,42,61,93,113))
...
scene.frame_set(round(113.25*demo.FPS)+1)
assert all(abs(c.location.z-.035)<1e-6 for c in clips)
scene.frame_set(round(14*demo.FPS)+1)
bpy.context.view_layer.update()
uvs = [world_to_camera_view(scene,scene.camera,p.co) for p in keys.key_blocks["Ten turns"].data if abs(p.co.x)<=demo.LENGTH/2]
width = max(p.x for p in uvs)-min(p.x for p in uvs)
assert .35<width<.52,("winding screen fraction",width)
for endpoint in (keys.key_blocks[0].data[0],keys.key_blocks[0].data[-1]):
    uv = world_to_camera_view(scene,scene.camera,endpoint.co)
    assert uv.x<0 or uv.x>1,"Supply endpoint visible during winding"
...
scene.frame_set(1)
print("PASS: initial OFF; five-second winding at half-screen; 10/20 turns; fixed current; reversal; smaller compasses; iron-core gain; magnetic regions; clips lift/fall; summary and packed application pictures.")
