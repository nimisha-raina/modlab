...
rings = sorted([obj for obj in scene.objects if obj.name.startswith("Magnetic concentric circle")], key=lambda obj: obj.name)
...
# The wide-view markers must never become physically larger than the particles
# in the magnified wire. Include the parent's scale in this world-space check.
for seconds in (7, 24, 36, 41):
...
for seconds, expected in [(0, False), (8.95, False), (9, True), (43, True), (44, False), (47, False)]:
    scene.frame_set(frame_at(seconds))
    if expected:
        assert abs(switch.rotation_euler.y) < 0.0001
    if not expected:
        assert all(ring.hide_render and ring.hide_viewport for ring in rings)
...
for frame in range(1, scene.frame_end+1):
    ...
    if map_time(5,timing) <= seconds <= map_time(32,timing):
    ...
    if seconds < map_time(LESSON.switch_on,timing):
        assert all(ring.hide_render for ring in rings)
...
scene.frame_set(1)
print(f"PASS: {len(scene.objects)} objects; 3 concentric circles; continuous camera; field timing; particle motion; caption bounds.")
